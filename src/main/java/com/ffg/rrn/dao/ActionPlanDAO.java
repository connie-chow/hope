...
public class ActionPlanDAO extends JdbcDaoSupport {
	...
	private static final String SQL_INSERT_ACTION_PLAN = "INSERT INTO ACTION_PLAN (RESIDENT_ID, PLAN_OF_ACTION, PLAN_DETAILS, REFERRAL_PARTNER, ANTICIPATED_OUTCOMES, ANTICIPATED_DATE, OUTCOME_ACHIEVED,  ACHIEVED_SSM, COMPLETION_DATE, FOLLOWUP_NOTES, SERVICE_COORD) VALUES (?,to_json(?::json),to_json(?::json), to_json(?::json),to_json(?::json),to_json(?::json), to_json(?::json), to_json(?::json), to_json(?::json),?,?)";

	private static final String SQL_UPDATE_ACTION_PLAN = "UPDATE ACTION_PLAN SET PLAN_OF_ACTION = to_json(?::json), PLAN_DETAILS = to_json(?::json), REFERRAL_PARTNER = to_json(?::json), ANTICIPATED_OUTCOMES = to_json(?::json), ANTICIPATED_DATE = to_json(?::json), OUTCOME_ACHIEVED = to_json(?::json), ACHIEVED_SSM = to_json(?::json), COMPLETION_DATE = to_json(?::json), FOLLOWUP_NOTES = ?, ANTICIPATED_OUTCOMES_DETAILS = to_json(?::json), DATE_MODIFIED = NOW() WHERE RESIDENT_ID = ? and DATE_ADDED = ? ";
	...
	private PreparedStatement buildUpdateActionPlan(Connection connection, @Valid Resident resident,
		...
		ps.setDate(12, parseMyDate(resident.getSelectedDate()));
		ps.setString(10, resident.getAnticipatedOutcomeDetails());  // new input fields seniorHealthIssue evictionAmount
		return ps;
	}
